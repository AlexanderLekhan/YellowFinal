...
void TestParseCondition() {
  ...
  }
  {
    istringstream is(R"(event <= "holiday" AND date <= 2017-11-18)");
    shared_ptr<Node> root = ParseCondition(is);
    Assert(root->Evaluate({2017, 11, 18}, "holiday"), "Parse condition 31");
    Assert(!root->Evaluate({2017, 11, 18}, "work day"), "Parse condition 32");
    Assert(root->Evaluate({1, 1, 1}, "ho"), "Parse condition 33");
    Assert(!root->Evaluate({2018, 11, 18}, "ho"), "Parse condition 34");
  }
}
