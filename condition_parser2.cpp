...
template <class It> shared_ptr<Node2> ParseComparison(It& current, It end) {
  if (current == end) {
  ...
  if (column.value == "date") {
    ...
    return make_shared<DateComparisonNode2>(cmp, ParseDate(is));
  } else {
    return make_shared<EventComparisonNode2>(cmp, value);
  }
}

template <class It>
shared_ptr<Node2> ParseExpression(It& current, It end, unsigned precedence) {
  if (current == end) {
    return shared_ptr<Node2>();
  }
  ...
  while (current != end && current->type != TokenType::PAREN_RIGHT) {
    ...
    left = make_shared<LogicalOperationNode2>(
