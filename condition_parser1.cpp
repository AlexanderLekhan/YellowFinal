...
template <class It> shared_ptr<Node1> ParseComparison(It& current, It end) {
  if (current == end) {
  ...
  if (column.value == "date") {
    ...
    return make_shared<DateComparisonNode1>(cmp, ParseDate(is));
  } else {
    return make_shared<EventComparisonNode1>(cmp, value);
  }
}

template <class It>
shared_ptr<Node1> ParseExpression(It& current, It end, unsigned precedence) {
  if (current == end) {
    return shared_ptr<Node1>();
  }
  ...
  while (current != end && current->type != TokenType::PAREN_RIGHT) {
    ...
    left = make_shared<LogicalOperationNode1>(
